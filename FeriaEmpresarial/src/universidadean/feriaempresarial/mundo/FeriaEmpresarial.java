...
public class FeriaEmpresarial {
   ...
   private int numExpositores (String nombreEmpresa){
     ...
     * @return respuesta1
     */
    public String metodo1() { //aca inicia
        //return "Respuesta 1";

        /*Zona Occidente.Obtener numero de expositores*/
        int contarOccidente=0;
        int contarCentro=0;
        int contarNorte=0;
        int contarSur=0;
        int contarOriente=0;
        for (Puesto p : puestos) {
            if (p.estaOcupado()) {
                if (p.darZona().equals(Puesto.ZONA_OCCIDENTE)) {
                    contarOccidente += numExpositores(p.darNombreEmpresa());
                }
            }
        }
        /*Zona Centro.Obtener numero de expositores*/
        for (Puesto p : puestos) {
            if (p.estaOcupado()) {
                if (p.darZona().equals(Puesto.ZONA_CENTRO)) {
                    contarCentro+= numExpositores(p.darNombreEmpresa());
                }
            }
        }
        /*Zona Norte.Obtener numero de expositores*/
        for (Puesto p : puestos) {
            if (p.estaOcupado()) {
                if (p.darZona().equals(Puesto.ZONA_NORTE)) {
                    contarNorte+= numExpositores(p.darNombreEmpresa());
                }
            }
        }
        /*Zona Sur. Obtener numero de expositores*/
        for (Puesto p : puestos) {
            if (p.estaOcupado()) {
                if (p.darZona().equals(Puesto.ZONA_SUR)) {
                    contarSur+= numExpositores(p.darNombreEmpresa());
                }
            }
        }
        /*Zona Oriente.Obtener numero de expositores*/
        for (Puesto p : puestos) {
            if (p.estaOcupado()) {
                if (p.darZona().equals(Puesto.ZONA_ORIENTE)) {
                    contarOriente+= numExpositores(p.darNombreEmpresa());
                }
            }
        }
        /*Zona Occidente. Comparar valores para saber si es el mayor*/

        if ( contarOccidente >= contarCentro && contarOccidente >= contarNorte && contarOccidente >=
        contarSur && contarOccidente >= contarOriente){
            return "Zona Occidente";
        }
        /*Zona Centro. Comparar valores para saber si es el mayor*/
        if ( contarCentro >= contarOccidente && contarCentro >= contarNorte && contarCentro >=
                contarSur && contarCentro >= contarOriente){
            return "Zona Centro";
        }
        /*Zona Norte. Comparar valores para saber si es el mayor*/
        if ( contarNorte >= contarOccidente && contarNorte >= contarCentro && contarNorte >=
                contarSur && contarNorte >= contarOriente) {
            return "Zona Norte";
        }
        /*Zona Sur. Comparar valores para saber si es el mayor*/
            if ( contarSur >= contarOccidente && contarSur >= contarCentro && contarSur >=
                    contarNorte && contarSur >= contarOriente) {
                return "Zona Sur";
            }
        /*Zona Oriente. Comparar valores para saber si es el mayor*/
                if ( contarOriente >= contarOccidente && contarOriente >= contarCentro && contarOriente >=
                        contarNorte && contarOriente >= contarSur) {
                    return "Zona Oriente";
                }
